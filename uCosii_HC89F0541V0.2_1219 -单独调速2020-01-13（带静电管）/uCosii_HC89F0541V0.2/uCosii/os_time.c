...
void  OSTimeDly (INT32U ticks) reentrant
{
    ...
    if (OSIntNesting > 0u) {                     /* 中断服务程序不能延时                  */
        return;
    }
    if (OSLockNesting > 0u) {                    /* 如果调度器被上锁不能延时，因为延时后就要进行调度           */
        return;
    }
    if (ticks > 0u) {                            /* 如果延时时间大于0才会进行延时                      */
        OS_ENTER_CRITICAL();
        y            =  OSTCBCur->OSTCBY;        /* 在就绪表和就绪组中取消当前任务的就绪标志                       */
        OSRdyTbl[y] &= (OS_PRIO)~OSTCBCur->OSTCBBitX;
        if (OSRdyTbl[y] == 0u) {
            OSRdyGrp &= (OS_PRIO)~OSTCBCur->OSTCBBitY;
        }
        OSTCBCur->OSTCBDly = ticks;              /* 给任务块的OSTCBDly项赋值延时时间                  */
        OS_EXIT_CRITICAL();
        OS_Sched();                              /* 进行一次任务调度*/                     
    }
}
